...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def __getitem__(self, idx):
        ...
        # if idx is out of your “valid” range, pick another index right away
        if idx > 12435:
            idx = self._rand_another(idx)

        while True:
            # only call prepare_train_data when idx <= 12435
            if idx <= 12435:
                data = self.prepare_train_data(idx)
            else:
                # if the newly‐randomized idx is still too big, pick again
                idx = self._rand_another(idx)
                continue
    ...
    # def __len__(self):
    #     # so DataLoader only ever samples 0…12435
    #     return 12436
